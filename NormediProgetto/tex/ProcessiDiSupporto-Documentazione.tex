...
Per la condivisione di materiale informale e/o non versionabile si predilige invece una cartella condivisa sullo spazio cloud offerto da \glossario{Google Drive}{Google Drive}. Le sopracitate tecnologie sono approfondite nel capitolo \textit{Strumentazione} di questo documento.

\subsection{Controllo della configurazione}

Per identificare e tracciare le richieste di cambiamenti, analizzare e valutare le modifiche effettuate e approvare o meno le quelle proposte, viene utilizzato il sistema di \glossario{issue}{issue} di Github. Le issue riportano nella loro descrizione la ragione della modifica effettuata, nonché la o le \glossario{commit}{commit} ad esse associate ed eventuali ulteriori issue correlate. 
...
\begin{itemize}
    ...
    \item \textbf{Oggetto:} indica, per gli obiettivi di prodotto, se esso si riferisce a documentazione o a software. Può assumere i valori:
...
\begin{itemize}
    ...
    \item \textbf{Oggetto:} indica, per le metriche di prodotto, se esso si riferisce a documentazione o a software. Può assumere i valori:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Orario di fine;
        \item Assenti(se presenti);
        \item Partecipanti esterni(nel caso fosse un incontro esterno);
    \end{itemize}
    \item \textbf{Resoconto:} contenente annotazioni riguardo gli argomenti discussi ed eventuali decisioni prese.
\end{itemize}
...
\subsection{Strumenti}
	...
	Per la stesura della documentazione il gruppo ha utilizzato il linguaggio \LaTeX{} per via delle possibilità che esso offre:
	
		\begin{itemize}
			\item creazione di documenti formali e divisi in sezioni molto velocemente;
			\item separazione del contenuto dalla formattazione tramite un file template separato e condiviso da tutti i documenti;
			\item personalizzazione del documento grazie all’elevato numero di librerie.
		\end{itemize}

 	\subsubsection{TexStudio}
 	
 	Per la stesura del codice \LaTeX{} viene utilizzato l’editor \glossario{TexStudio}{TexStudio}, in virtù delle innumerevoli feature che offre gratuitamente e del fatto che si tratta di un'applicazione cross-platform supportata dai maggiori sistemi operativi. \\
 	\centerline{\url{http://www.texstudio.org/}}
